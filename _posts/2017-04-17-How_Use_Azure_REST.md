...
I use Azure a lot to create virtual machines to host demo or prototype applications. It often takes me a long time to setup my applications and all the services that support them. Once I finally get things setup the way I like them, I really don't want to duplicate that effort, so cloning VMs is something I do a lot. However, Azure doesn't really make it easy to automate the process of cloning a VM. In my case, it's even harder because the applications I build are designed to run on clusters, such as Hadoop or MapR. At a minimum I need to provision 3 VMs (that's a 3-node cluster) with tens of GBs of data in secondary disk storage, and more hostname based service configurations than you can shake a stick at. To set this up every time a sales rep asks me for a new demo rig would be completely unsustainable. It's FAR easier to setup a single demo rig and clone it in Azure. So, that's what I'm going to describe how to do here. Or more specifically, I'm going to brain dump my notes on how cloning is supposed to work, and how to troubleshoot when it doesn't work.
