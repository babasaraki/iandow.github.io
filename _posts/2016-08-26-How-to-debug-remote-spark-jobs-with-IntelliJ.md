...
Application developers often use debuggers to find and fix defects in their code. Attaching a debugger to running application is straightforward when the runtime is local on a laptop but it's a bit trickier when that code runs on a remote server. To make matters worse, Big Data applications are designed to run in a distributed fashion across a multi-node cluster. This represents a challenge for debugging since you have to attach a debugger to a process that's running on multiple remote cluster nodes. Fortunately, for Apache Spark that's actually not that hard.
